...
const ContentCard = ({ title, location, price, uploaded_images, amenities }: Listing) => {
  const imageUrls = uploaded_images?.map(img => img.image) ?? [];
  const [selectedImage, setSelectedImage] = useState(imageUrls[0] ?? "/frontend/public/next.svg");
          ...
          <div className="p-1 pl-1.5 sm:p-4 flex flex-col justify-between gap-1.5 sm:gap-4 h-full rounded-r-2xl text-xs sm:text-sm">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 text-[11px] sm:text-xs py-2 ">
                {amenityList.slice(0, 3).map(({ label }, idx) => {
                 return (
          ...
          <div className="flex sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-5 border-t border-gray-200 pt-1">
            ...
            >
              <Heart
                ...
                strokeWidth={2}
                className={`transition-transform duration-200 ${wishlisted ? "scale-110" : "scale-100 hover:scale-105"
                  }`}
              />
